# Results
## Museum in US
### The distribution of museums of different discipline
...
We can find that HSC(Historical Societies / Historic Preservation) and GMU(Uncategorized or General Museums) take great share in each state's museums, especially HSC. And the proportions of different disciplines in different states remains similar, which means there is no tendency of a state to own one kind of museum more.
...
It shows that there are a lot of museums did not provide their income status. In those museums that provided incomes, except ART and CMU, museums of other disciplines that did not have incomes takes biggest share, especially HSC, since most of this kind of museum don't need tickets. 

And also incomes between 25,000 and 4,999,999 take some shares. But since Art Museums are often charge highly, there are great proportion of Art museums earn income over 50,000,000 in 2015.
...
df_year<-museum_c %>%
  ...
  mutate(decate=floor(rule_year/10)*10)%>%
  group_by(decate, DISCIPLINE)%>%
  summarise(number=n())

ggplot(df_year, aes(x=decate, y=number, group= DISCIPLINE, color= DISCIPLINE)) +
  geom_line()
```

The numbers of most kinds of museums increase slowly since 1920s to 2000s. Since the data is collected in 2018, the new museum numbers cannot present the total 2010s. However, there is an obvious increase of HSC number from 1950s to 2000s, which indicates that the government established lots of Historical Societies / Historic Preservation after 1950s. 

Besides that, the numbers of GMU and HST also increased a lot. The rapid growth of history related museum reflects that we care about our history more and more in recent days.
Also, the number of art museum in 1940s has an obvious increase, which means people at that time the fasion of enjoying art was prevailing.
...
popstate %>%
  ...
  geom_point() +
  ggtitle("Population, Number of People Visit Library and Number of Registers in Different States") +
  ylab("") +
  theme_linedraw()
```
...
gnumofLi<-ggplot(data = numliPostate_tidy, aes(x = factor(STABR, levels = order$STABR), y = numberoflibraires, fill = librarytype)) +
  ...
  ggtitle("Number of Libraries in Different States Ordered by State's Poluation") +
...
gnumofLi<-ggplot(data = numliPostate_tidy, aes(x = factor(STABR, levels = order2$STABR), y = numberoflibraires, fill = librarytype)) +
  ...
  ylab("Number of Library") +
  guides(fill=guide_legend(title="Library Type")) +
  ggtitle("Number of Libraries in Different States") +
  theme(plot.title = element_text(hjust = 0.5,size=15)) +
  scale_y_continuous() +
  coord_flip()
gnumofLi
...
ggplot(data = numCostate_tidy, aes(x = factor(STABR, levels = order2$STABR), y = numberofresource, fill = resourcetype)) +
  ...
  ggtitle("Resource in Different States' Library Ordered by Number of Library") +
  theme(plot.title = element_text(hjust = 0.5,size=15)) +
  scale_y_continuous() +
  coord_flip()
```
...
ggplot(data = numEvstate_tidy, aes(x = factor(STABR, levels = order2$STABR), y = numberofevent, fill = eventtype)) +
  ...
  ggtitle("Events in Different States' library Ordered by Number of Library") +
  theme(plot.title = element_text(hjust = 0.5,size=15)) +
  scale_y_continuous() +
  coord_flip()
```
...
ggplot(data = revstate_tidy, aes(x = factor(STABR, levels = order2$STABR), y = amountofrevenue, fill = revenuetype)) +
  ...
  ggtitle("Amount of Revenue Received by Different States' Librariey Ordered by Number of Library") +
  theme(plot.title = element_text(hjust = 0.5,size=15)) +
  scale_y_continuous() +
  coord_flip()
```
...
state_choropleth(df_museum_c,
                 title = "State Museum Number, 2020",
...
#df
state_choropleth(df,
                 title = "State Library Number, 2020",
